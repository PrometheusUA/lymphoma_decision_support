...
def get_orlov_datasets(orlov_dataset_folder=ORLOV_DATASET_FOLDER):
    ...
    assert os.path.isfile(f"{orlov_dataset_folder}/patients.csv"), "Patients.csv doesn't exist!"
    patients_df = pd.read_csv(f"{orlov_dataset_folder}/patients.csv")
    ...
    for subset_class in data.datasets[0].classes:
        ...
        val_split_id = test_split_id + max(1, int(np.floor(VAL_SPLIT * patients_class_count)))
    ...
    for dataset_num in range(len(data.datasets)):
        for filename in train_files:
            filename_index = indices[np.char.find(images_array, filename) >= 0] + len(images_array) * dataset_num
    ...
    train_loader = DataLoader(data, batch_size=BATCH_SIZE, 
                                            sampler=train_sampler, num_workers=NUM_LOADERS_WORKERS, persistent_workers=True)
    val_loader = DataLoader(data, batch_size=BATCH_SIZE,
                                                    sampler=val_sampler, num_workers=NUM_LOADERS_WORKERS, persistent_workers=True)
    test_loader = DataLoader(data_test, batch_size=BATCH_SIZE, 
                                                sampler=test_sampler, num_workers=NUM_LOADERS_WORKERS, persistent_workers=True)
