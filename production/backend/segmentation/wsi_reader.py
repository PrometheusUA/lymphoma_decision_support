...
class WSIReader:
    ...
    def read_region(
        self, x_y, level, tile_size, normalize=True, downsample_level_0=False
    ):
    ...
    def read_region_ds(
        self, x_y, downsample, tile_size, normalize=True, downsample_level_0=False
    ):
        if isinstance(tile_size, int):
            tile_size = (tile_size, tile_size)

        level = (
            0 if downsample_level_0 else self.get_best_level_for_downsample(downsample)
        )
        x_y_level = [
            round(coord * downsample / self.level_downsamples[level]) for coord in x_y
        ]
        tile_size_level = [
            round(dim * downsample / self.level_downsamples[level]) for dim in tile_size
        ]
        tile, alfa_mask = self.read_region(
            x_y_level, level, tile_size_level, False, False
        )
    ...
    def get_downsampled_slide(self, dims, normalize=True):
        ...
        slide_downsampled, alfa_mask = self.read_region(
            (0, 0), level, self.level_dimensions[level], normalize=normalize
        )
        slide_downsampled = cv2.resize(
            slide_downsampled, dims, interpolation=cv2.INTER_CUBIC
        )
        alfa_mask = cv2.resize(
            alfa_mask.astype(np.uint8), dims, interpolation=cv2.INTER_CUBIC
        ).astype(np.bool)
...
class OpenSlideReader(WSIReader):
    ...
    @property
    def mpp(self):
        return float(self._slide.properties["openslide.mpp-x"]), float(
            self._slide.properties["openslide.mpp-y"]
        )
    ...
    def tile_dimensions(self):
        if not hasattr(self, "_tile_dimensions"):
            ...
            for level in range(self.level_count):
                tile_width = self._slide.properties[
                    f"openslide.level[{level}].tile-width"
                ]
                tile_height = self._slide.properties[
...
class TiffReader(WSIReader):
    ...
    def _read_region(self, x_y, level, tile_size):
        ...
        planes, alfa_mask = zip(
            *[self._read_page_region(page, x_y, tile_size) for page in level.pages]
        )
        ...
        if alfa_mask.ndim == 3 and (
            planes.ndim == 2 or alfa_mask.shape[2] != planes.shape[2]
        ):
    ...
    def _read_page_region(self, page, x_y, tile_size):
        ...
        if not page.is_tiled:
            ...
            tile, alfa_mask = self._get_alfa_mask(
                tile, page.samplesperpixel, page.extrasamples
            )
            return tile, alfa_mask
...
def get_reader_impl(slide_path):
    if Path(slide_path).suffix == ".ndpi":
